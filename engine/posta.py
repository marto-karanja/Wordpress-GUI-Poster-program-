...
class Posta():
    ...
    def post_content(self, workload):
        ...
        # initialize cleaner
        cleaner = Cleaner()
        posts = db.fetch_posts( category = workload['category'], posts = workload['posts'], table = workload['table'] )
        #
        short_content = []
        ...
        for post in posts:
            ...
            content = cleaner.clean_content(post['processed_content'])
            if content:
                ...
                self.logger.debug("Post No: [%s] updated", post['link_no'])
            else:
