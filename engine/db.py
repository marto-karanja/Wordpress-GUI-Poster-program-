...
class Db(object):
    ...
    def fetch_category_posts_from_tables(self, no_of_posts = None, categories = None):
        ...
        for table in categories.keys():
            # fetch random
            if post_no != 0:
                query = "select link_no, title, content, content_length, category from " + table + " where processed = 'False' and content_length > 80 and category in ('"+ "','".join(categories[table]) + "') ORDER BY RAND()"
